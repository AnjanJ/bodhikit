...
Claude Code loads `skills/`, `agents/`, `hooks/`, and the `.claude-plugin/` manifests from an installed plugin — and **nothing else**: a `knowledge/` tree is never registered and a plugin `rules/` directory is never loaded (both verified with headless probes in 1.18.0; from 1.0 to 1.17 every "Reference the `X` KB" pointed at a file the runtime could not load). Since 1.18.0 the 18 knowledge bases live under `skills/<kb>/SKILL.md` with `user-invocable: false` — registered, hidden from the `/` menu, loaded on demand through the Skill tool — and `rules/learning-project.md` is delivered by the SessionStart hook (`scripts/bodhi-session-context.py`). `scripts/` ships with the install and is executed via Bash (it never enters context). Root-level files (this CLAUDE.md, README, CHANGELOG, `dev/`) are inert at install time.
