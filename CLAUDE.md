...
- Learner-facing text never quotes a raw Bloom number or Leitner box. Levels render as `**Label** — outcome clause` from the `blooms-taxonomy` KB *Learner-Facing Rendering* table (labels stay because a named rung motivates; the clause is what makes it mean something); `bodhi-state` emits `bloomLabel`/`bloomOutcome` next to every `bloomLevel` so skills render rather than translate. Module rollups use the tier words (*Solid / Working / Introduced*). The two sanctioned exceptions are listed in that KB.
- `dev/check.sh` admits only **structural** or **conditional** rules (policy in its header). A rule that greps a KB name into a phase is not a test; behaviour goes in `dev/eval/`. Rule numbers are stable identifiers and are not contiguous.
