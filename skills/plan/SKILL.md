...
2. Build a new plan following `/learn` Phase 3 (plan principles — ZPD calibration, spiral curriculum, spaced reinforcement) PLUS `/learn` Phase 4 (sectional v2 layout: `plan/README.md` + per-phase `plan/phase-{N}.md`). Each phase after Phase 0 must declare at least one Spiral Revisit per the `constructivism` KB — a concept from an earlier phase reappearing at a higher target Bloom level.
