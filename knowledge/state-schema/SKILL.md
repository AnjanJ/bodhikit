...
**Legacy fallthrough (v2 → v3).** Concepts migrated from v2 receive `bloomLevel: 0`, `feynmanPassed: false`, `consecutiveCorrectAtL4Plus: 0`. **A concept with `bloomLevel: 0` is a concept no v3 writer has classified yet** — it may have a populated `lastReviewed` from pre-v3 quizzes (real v2 data routinely does), but the v3 `bloomLevel` field has never been written to. Skills checking prerequisite Bloom gates MUST treat `bloomLevel: 0` as "no opinion yet, allow advancement" rather than "stalled at zero, block." `lastReviewed` is NOT part of this check — only the absence of a v3 `bloomLevel` write matters. Once any v3 writer (`/quiz`, `/teach`, `/explain`, `/practice`) writes a non-zero `bloomLevel`, normal gate logic applies to that concept.

The same boundary applies to displays. `/progress` Mastery % treats a module where every concept has `bloomLevel: 0` as "—" (not computable yet), regardless of whether `lastReviewed` is populated; once at least one concept in the module has `bloomLevel > 0`, the formula computes against the v3-classified subset and the legacy concepts count as not-yet-mastered.

**Historical note.** The 1.10.0 release of this fallthrough rule used `bloomLevel: 0 AND lastReviewed: null` as the legacy-state predicate. Dogfooding against real v2 data revealed that pre-v3 concepts routinely have populated `lastReviewed` from earlier quizzes — the rule false-blocked module advancement on every existing learner. 1.10.7 dropped `lastReviewed` from the check; the v3 `bloomLevel` field's value alone identifies whether the concept has been classified under the new schema.
