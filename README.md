...
**An honest note on the claim.** BodhiKit's *design* follows the learning-science literature below — each mechanism cites its primary source and fires where the research says it should. What we do not (yet) have is outcome data showing the plugin itself improves learning versus vanilla Claude; the thresholds and intervals are tuned on real but small-N usage. "Research-informed" is a design claim, not an efficacy guarantee. If you run real learning projects through BodhiKit and want to help change that, open an issue with your (anonymized) experience — what held, what decayed, where the calibration felt off. As of 1.11.3 this is one command: `bodhi-state export-anonymized` produces a shareable stats block (counts and rates only — no concept names, no free text), and the repo has a "Share your learning data" issue template for it. `bodhi-state retention` shows you the same numbers privately: your %-correct at review time, by spacing gap and by box — the empirical check on whether the intervals are working *for you*.

**Evidence tiers (1.11.2).** The citations below do not all carry equal weight, and each knowledge base now declares which tier it sits in. The load-bearing mechanisms — retrieval practice, spacing, pretesting, worked examples, successive relearning — sit on the most-replicated effects in the learning literature (John Dunlosky et al., *[Improving Students' Learning With Effective Learning Techniques](https://journals.sagepub.com/doi/10.1177/1529100612453266)*, 2013, rates practice testing and distributed practice as the two highest-utility techniques of the ten reviewed). Bloom's, ZPD, and GROW are organizing frameworks — structure, not measured interventions. And two citations are kept with explicit caveats: growth-mindset interventions show small-to-null average effects in meta-analyses (Sisk et al. 2018 — BodhiKit uses only the low-cost language patterns), and deliberate practice's domain-generality claims are contested (Macnamara et al. 2014 — BodhiKit uses only the uncontested core: targeted practice with immediate feedback). Fencing the weak claims is what lets you trust the strong ones.
